...
<div class="absolute top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center">
 <form action='/company/register' method="POST" class="relative bg-white p-10 rounded-xl text-slate-700 " enctype="multipart/form-data">
       @csrf <!-- CSRF Token for security -->
   <h1 class="text-center text-2xl text-neutral-700 font-medium"> Campany Register </h1>
   <p class="text-sm text-center">Welcome, please Register to continue. </p>
     ...
     <div class="border px-4 py-2 flex items-center gap-2 rounded-full mt-5">
      ...
      <input name="name" class="outline-none text-sm w-full" placeholder="Write Company Name" required />
    </div>
    <div class="">
      @error('name')
    ...
    <!-- EMAIL INPUT -->
      <div class="border px-4 py-2 flex items-center gap-2 rounded-full mt-5">
        ...
        <input name="email" class="outline-none text-sm w-full" placeholder="Write Company Email Address" required />
      </div>
      <div class="">
        @error('email')
    ...
    <p class="mt-5 text-center">Already have an account?
      <a  href="/company/login" class="text-blue-600 cursor-pointer"  >Login </a>
    </p>
 </form>
</div>
</x-PageLayout>
